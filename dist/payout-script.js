...
async function sendSlackMessage(eraStats) {
    try {
...
const main = async () => {
    ...
    for (let currentEra = startEra; currentEra <= activeEra - 3; currentEra++) {
        ...
        const eraRewardPoints = (await api.query.staking.erasRewardPoints(currentEra)).toJSON();
        ...
        const erasValidatorPrefs = (await api.query.staking.erasValidatorPrefs.entries(currentEra)).map(([key, value]) => {
            ...
            const { commission, blocked } = value.toHuman();
            return {
                individual,
                commission,
                blocked
        ...
        const erasStakersOverview = (await api.query.staking.erasStakersOverview.entries(currentEra)).map(([key, value]) => {
            const keyHuman = key.toHuman();
            const [, individual] = keyHuman;
            const { total, own, nominatorCount, pageCount } = value.toHuman();
            ...
            const commissionDecimal = Number(validatorPref?.commission?.replace('%', '')) / 100 || 0;
        ...
        const claimedRewards = (await api.query.staking.claimedRewards.entries(currentEra)).map((x) => x[0].toHuman()[1]);
